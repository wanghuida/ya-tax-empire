...
//links="[{left:'30px',top:'81px'},{left:'30px',top:'244px'},{direction:'t
...
indexSlides.ini = function () {
    ...
    $(window).resize(function () {
        ...
        yatax.page.autoMaxWidth();
    });

}

indexSlides.goto = function (i) {
    if (indexSlides.animating || i == indexSlides.current) return false;

    indexSlides.animating = true;

    var j = i;
    if (i >= indexSlides.style.length) j = 0;
...
indexSlides.pause = function () {
    if (indexSlides.timer) clearInterval(indexSlides.timer);
}
...
indexSlides.clearStage = function (i) {
    if (indexSlides.current > -1) {
        indexSlides.animating = true;
        var left = 3000;
        if (i < indexSlides.current) left = -1000;
        indexSlides.obj.eq(indexSlides.current).find('.text, .button').animate({
            'left':left + 'px'
        }, 500, function () {
        });
    }
...
latestNews.pause = function () {
    if (latestNews.timer) clearInterval(latestNews.timer);
}

var yatax = {};
yatax.namespace = function (str) {
    var arr = str.split('.'),
        o = yatax;
    for (i = (arr[0] == 'yatax') ? 1 : 0; i < arr.length; i++) {
        o[arr[i]] = o[arr[i]] || {};
        o = o[arr[i]];
    }
}
yatax.namespace('page');

yatax.page.autoMaxWidth = function () {
...
yatax.page.onLoad = function () {
    ...
    if (l.indexOf('about-yatax') > -1) {
        $('#menu a.about_yatax').addClass('active');
        return $('#menu').data("normal", "about-yatax");
    }
